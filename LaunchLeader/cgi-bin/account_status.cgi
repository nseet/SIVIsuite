...
&Launchh::select_account_status($Launchh::account_status->{'account_status_id'}) if($Launchh::account_status->{'account_status_id'});
&Launchh::read_account_status() if($stage>=2 && $mode eq "edit");
...
elsif($stage==3)
{
	if($mode eq "delete")
	{
	   &Launchh::delete_account_status($Launchh::account_status->{'account_status_id'});
	}
	else
	{
	   if( !$Launchh::account_status->{'account_status_id'})
	   {
		&Launchh::insert_account_status();
	   }
	   else
	   {
		&Launchh::update_account_status();
	   }
	}



   print "Location: $Launchh::ACCOUNT_STATUSS_CGI?$Launchh::HSTR&stage=2\n\n";
}
...
<TABLE WIDTH="$Launchh::TABLE_WIDTH" BORDER="0">   
		...
		<TR><TD><FONT FACE="$Launchh::FONT" SIZE="$Launchh::SIZE">
                 Id: &nbsp; <B>$Launchh::account_status->{'account_status_id'}</B>
                </FONT></TD></TR>
		<TR><TD><FONT FACE="$Launchh::FONT" SIZE="$Launchh::SIZE">
                 Name: &nbsp; <B>$Launchh::account_status->{'account_status_name'}</B>
                </FONT></TD></TR>
...
<TABLE WIDTH="$Launchh::TABLE_WIDTH" BORDER="0">
   ...
   <TR>
	<TD>
	<FONT FACE="$Launchh::FONT" SIZE="$Launchh::SIZE">
	<FORM ACTION="$Launchh::ACCOUNT_STATUS_CGI" METHOD="POST">
...
<TABLE WIDTH="$Launchh::TABLE_WIDTH" BORDER="0">
   <TR>
	...
	<TABLE BORDER="0" CELLPADDING="3">
		...
		<TR><TD><FONT FACE="$Launchh::FONT" SIZE="$Launchh::SIZE">
		Id: &nbsp; <B>$Launchh::account_status->{'account_status_id'}</B>
		<INPUT TYPE="hidden" NAME="account_status_id" value="$Launchh::account_status->{'account_status_id'}">
		</FONT></TD></TR>
		<TR><TD><FONT FACE="$Launchh::FONT" SIZE="$Launchh::SIZE">
		Name: &nbsp; <B>$Launchh::account_status->{'account_status_name'}</B>
		<INPUT TYPE="hidden" NAME="account_status_name" value="$Launchh::account_status->{'account_status_name'}">
		</FONT></TD></TR>
...
sub verify_stage1
{
#		if($Launchh::account_status->{'account_status_id'} eq "" || ($Launchh::account_status->{'account_status_id'} =~ /\D/is))
#		{
#		$Launchh::ERROR .= "Please enter a valid Id<BR>";
#		return 0;
#		}
#		if($Launchh::account_status->{'account_status_name'} eq "" )
#		{
